<?php
	if ($_GET['classe'] != ""){
		...
		$presenze = array();
		$return = mysql_query("SELECT * FROM Presenza$giorno");
		foreach($member as $value){
			$presenze[$value['id']] = array();
		}
		while($row = mysql_fetch_array($return)){
			$presenze[$row['user']][$row['turno']] = $row['id_forum'];
		}
				...
				<?php
					...
					<td><?php 
						if ($value[0] != "" && $value[0] < 90){
							if (count($presenze[$value['id']]) == "2" && $presenze[$value['id']][2] == $presenze[$value['id']][1] && $presenze[$value['id']][1] == $value[0]){
								echo "Presente";
							}else{
								switch ($value[0]){
									case $presenze[$value['id']][1]:
										echo "Presente solo prima della ricreazione";
									break;
									case $presenze[$value['id']][2]:
						...
						echo base64_decode($list_forum[$value[0]]); ?><?php echo $classi[$value[0]];?></td>
					<td><?php 
						if ($value[1] != "" && $value[1] < 90){
							if ($presenze[$value['id']][1] == $value[1]){
								echo "Presente";
							}else{
								echo "Non presente";
							}
						...
						echo base64_decode($list_forum[$value[1]]); ?><?php echo $classi[$value[1]];?></td>
					<td><?php 
						if ($value[2] != "" && $value[2] < 90){
							if ($presenze[$value['id']][2] == $value[2]){
								echo "Presente";
							}else{
								echo "Non presente";
							}
						...
						echo base64_decode($list_forum[$value[2]]); ?><?php echo $classi[$value[2]];?></td>
				</tr>
						<?php
					}
				?>
		...
		<div id="box"><center>
			...
			var attuale = 1;
				function get(giorno,g){
					...
					xmlhttp.open("GET","http://studenti.liceoblaisepascal.it/index.php?page=admin&w=classe&classe="+giorno+"&g="+g.toString(),true);
					xmlhttp.send();
			}
			</script>
